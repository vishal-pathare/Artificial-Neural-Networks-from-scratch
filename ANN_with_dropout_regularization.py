...
class ANN:
    ...
    def implement_dropout(self, A_prev):
    ...
    def linear_forward(self, A_prev, W, b, i):

        #Calculates and returns Z

        if i != 1:
            A_prev, D = self.implement_dropout(A_prev)
        else:
            D = np.ones(A_prev.shape)
        Z = np.dot(W, A_prev) + b
        cache = (A_prev, W, b, D)
        return Z, cache

    def activation_forward(self, A_prev, W, b, activation, i):

        #Calculates activation of layer

        Z, cache = self.linear_forward(A_prev, W, b, i)
    ...
    def L_layers_forward(self, X, parameters):
        ...
        for i in range(1, L):
            A_prev = A
            A, cache = self.activation_forward(A_prev, parameters["W" + str(i)], parameters["b" + str(i)], "relu", i)
            caches.append(cache)
        AL, cache = self.activation_forward(A, parameters["W" + str(L)], parameters["b" + str(L)], "sigmoid", i)
        caches.append(cache)
        return AL, caches
    ...
    def train(self, X, Y, parameters, learning_rate = 0.075, no_of_iterations = 1000, print_cost = False, batch_size = 64, optimizer = "gradient_descent_optimizer", keep_prob = 0.8):
